...
import xin.zhuyao.wechatbotmultiple.protocol.*;
import xin.zhuyao.wechatbotmultiple.utils.WXHttpExecutor;
...
final class WeChatApi {
    ...
    File webwxgetmsgimg(long msgId, String type) {
        ...
        File imgFile = XTools.http(httpExecutor, request).file(folder.getAbsolutePath() + "/static/jpg" + File.separator + String.format("image-%s-%s", String.valueOf(type), msgId));
        try {
            String suffix = WeChatTools.fileSuffix(imgFile);
            if (!XTools.strEmpty(suffix)) {
                File file = XTools.fileToFile(imgFile, folder.getAbsolutePath() + "/static/jpg" + File.separator + String.format("image-%s-%s.%s", String.valueOf(type), msgId, suffix));
                imgFile.delete();
                return file;
            }
        } catch (IOException e) {
    ...
    File webwxgetvoice(long msgId) {
        ...
        request.query("skey", skey);
        request.query("pass_ticket", passticket);
        return XTools.http(httpExecutor, request).file(folder.getAbsolutePath() + "/static/mp3" + File.separator + String.format("voice-%s.mp3", msgId));
    }
    ...
    File webwxgetvideo(long msgId) {
        ...
        return XTools.http(httpExecutor, request).file(folder.getAbsolutePath() + "/static/mp4" + File.separator + String.format("video-%d.mp4", msgId));
    }
    ...
    File webwxgetmedia(long msgId, String filename, String mediaId, String sender) {
        ...
        return XTools.http(httpExecutor, request).file(folder.getAbsolutePath() +"/static/unknown" + File.separator + String.format("media-%d%s", msgId, suffix));
    }
